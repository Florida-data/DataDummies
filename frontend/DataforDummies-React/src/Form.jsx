...
export function Form({ names, labels, types, classnames, title, title_class, class_button, textButton, onChangeFunction, onSubmit, errors, formData }) {
        ...
        return labels.map((label, index) => (
            <div key={index} className={`${classnames[3] || ''} form-group col-12 col-sm-8 col-md-6 col-lg-10 mx-0 px-0`}>
                ...
                <input 
                    ...
                    required
                />
                {errors && errors[names[index]] && (
                    <div className="error-message text-danger">{errors[names[index]]}</div>
                )}
            </div>
        ));
    };
    ...
    return (
        <form className={`${bootstrapClasses.join(" ")} col-12 mx-0 px-0`} onSubmit={onSubmit}>
            ...
            <div className="container-fluid col-12 mt-5 d-flex flex-row px-0 align-items-center flex-column">
