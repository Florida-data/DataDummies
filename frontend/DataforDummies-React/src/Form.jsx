...
export function Form({ labels, types, classnames, title, title_class, class_button, textButton }) {
    ...
    const label_forms = (labels, types) => {
        return labels.map((label, index) => (
            <div key={index} className={`${classnames[3]} form-group col-12 col-sm-8 col-md-6 col-lg-10 mx-0 px-0`}>
                ...
                <input type={types[index]} className={`${classnames[1]} form-control`} />
            </div>
        ));
    };

    return (
        <form className={`${bootstrapClasses.join(" ")} col-12 mx-0 px-0`}>
            {title && (
